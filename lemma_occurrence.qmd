...
myplot = freq_cut %>% # our data
    ...
    ggplot(mapping = aes(x = word,
                         y = n)) + 
    geom_col(fill = "grey", position = "stack") +
    coord_flip() + # flip x and y coordinates 
    labs(x = "Lemme",
         y = "Nombre d'occurrences",
         title = paste0(riviere, " et ", ville)) +
...
```{r}
c_fid = c("37", "51", "164", "185", "139", "65", "985")

df_tokens = read.csv("collected_data/english/df_tokens.csv")
df_tok2 = df_tokens %>% 
  select(c("fid", "urban_aggl", "river_en", "token", "snippet_id"))
df_tokens_hl = read.csv("collected_data/hl/df_tokens_all_hl.csv")
df_tok3 = df_tokens_hl %>% 
  select(c("fid", "urban_aggl", "river_en", "token", "snippet_id"))

combinaisons = df_tok2 %>% 
  rbind(df_tok3) %>% 
  filter(fid %in% c("37", "51", "164", "185", "139", "65", "985")) %>% 
  filter(token != "al") %>% 
  filter(token != "diyarbakır") %>% 
  group_by(token) %>% 
  count()
```
