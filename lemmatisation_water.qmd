...
The aim of this document is to lemmatise and clean our snippets for the queries "water AND city".
...
data_city_river = data_city_river %>% 
    ...
    distinct(data_city_river$urban_aggl, .keep_all = TRUE)
...
```{r def_lemmatisation}
lemmatisation_en = function(fid, cityname){
  
    file = glue::glue("collected_data/water_resource/english/tokens/tokens_{fid}.csv")
    if(!file.exists(file)){
      
      # read data
      df = read.csv(glue::glue("collected_data/water_resource/english/clean_data/clean_{fid}.csv")) 
      ...
      # define words to remove, i.e. the city name 
      fid_names = data.frame(word = cityname) %>% 
      ...
      # now get all spellings of the city name
      fid_names = fid_names %>% 
        mutate(ascii = tolower(stringi::stri_trans_general(word, "latin-ascii"))) %>%
        mutate(punct = tolower(gsub('[[:punct:]]+', '', word))) %>% 
        mutate(punct_ascii = tolower(gsub('[[:punct:]]+', '', stringi::stri_trans_general(word, "latin-ascii")))) %>% 
        mutate(punct2 = tolower(gsub('[[:punct:] ]+', ' ', word))) %>% 
        mutate(punct2_ascii = tolower(gsub('[[:punct:] ]+', ' ', stringi::stri_trans_general(word, "latin-ascii")))) 
      fid_names = fid_names %>% 
      ...
      write.csv(df_clean, glue::glue("collected_data/water_resource/english/tokens/tokens_{fid}.csv"), row.names = FALSE)
   }
}
```
...
lemmatisation_hl = function(fid, cityname, hl){
  
   file = glue::glue("collected_data/water_resource/hl/tokens/tokens_{fid}_{hl}.csv")
    if(!file.exists(file)){
      
      # read data
      if(file.exists(glue::glue("collected_data/water_resource/hl/clean_data/clean_{fid}_{hl}.csv"))){
      df = read.csv(glue::glue("collected_data/water_resource/hl/clean_data/clean_{fid}_{hl}.csv")) 
      ...
      write.csv(df_clean, glue::glue("collected_data/water_resource/hl/tokens/tokens_{fid}_{hl}.csv"), row.names = FALSE)
...
for(i in 1:6) {
  ...
  data = select(.data = data_subset, c(1:7, n_hl, 14, 34, 35))
