---
title: "Search Engine Pages Results: Collect data 'flood and city'"
format: html
editor: visual
...
get_serp_data = function(fid, cityname){
    file = glue::glue("collected_data/floods/english/raw_data/{fid}.rds")
    
          if(!file.exists(file)){
          # Parameters list
            q = paste0("flood AND (", cityname,")") # our query
            params = list(
...
get_serp_tibble = function(fid){
  ...
  file = glue::glue("collected_data/floods/english/raw_data_text/raw_{fid}.csv")

    if(!file.exists(file)){
# translate to string
    res = readRDS(file = glue::glue("collected_data/floods/english/raw_data/{fid}.rds"))
    ...
    readr::write_csv(res_tib,
                     glue::glue("collected_data/floods/english/raw_data_text/raw_{fid}.csv"))
    return(res_tib)
  }
}
```
...
translate_snippets = function(fid){
  ...
  file = glue::glue("collected_data/floods/english/clean_data/clean_{fid}.csv")
  
  if(!file.exists(file)){
    df = read.csv(glue::glue("collected_data/floods/english/raw_data_text/raw_{fid}.csv"))
  ...
  readr::write_csv(final_df,
                   glue::glue("collected_data/floods/english/clean_data/clean_{fid}.csv"))
  }
}
```
...
get_serp_data_hl = function(fid, cityname, gl, hl, google_domain){
  ...
  file = glue::glue("collected_data/floods/hl/raw_data/{fid}_{hl}.RDS")
         if(!file.exists(file)){
    ...
    # let's define our query with a parameters list
           if(google_hl == "de")
             ...
             flood = polyglotr::google_translate("flood", target_language = google_hl, source_language = "en")}
...
# translate to string
    ...
    file = glue::glue("collected_data/floods/hl/raw_data_text/raw_{fid}_{hl}.csv")
    
    if(!file.exists(file)){
      res = readRDS(file = glue::glue("collected_data/floods/hl/raw_data/{fid}_{hl}.RDS"))
      res_text <- httr::content(res, "text")
      ...
      readr::write_csv(res_tib,
                       glue::glue("collected_data/floods/hl/raw_data_text/raw_{fid}_{hl}.csv"))
...
translate_snippet = function(fid, hl){
  ...
  file1 = glue::glue("collected_data/floods/hl/clean_data/clean_{fid}_{hl}.csv")
  file2 = glue::glue("collected_data/floods/hl/clean_data/nodata_{fid}_{hl}.csv")
  
  if(!file.exists(file1) & !file.exists(file2)){
    if(file.exists(glue::glue("collected_data/floods/hl/raw_data_text/raw_{fid}_{hl}.csv"))){
      df_hl= read.csv(file = glue::glue("collected_data/floods/hl/raw_data_text/raw_{fid}_{hl}.csv")) 
     
      # check if data has been found beforehand
      if(df_hl[1,1] == "NoData"){
        print(paste0("No data, fid = ",fid, ", hl = ",hl))
        write.csv(df_hl, glue::glue("collected_data/floods/hl/clean_data/nodata_{fid}_{hl}.csv"))
      }
  ...
  write.csv(final_df, glue::glue("collected_data/floods/hl/clean_data/clean_{fid}_{hl}.csv"))
      }
    }
    else{
