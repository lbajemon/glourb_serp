...
setwd("~/01_thèse/SERP/1_glourb_serp_get_data")
```

## Description

The aim of this document is to use the ![](https://dka575ofm4ao0.cloudfront.net/pages-transactional_logos/retina/211386/logo_color_transparent_background.png){width="105" height="17"} API ([access link](https://get.valueserp.com/try-it-free/)) to collect data relative to Google searches. SERP stands for Search Engine Results Pages and corresponds to the pages displayed in response to a query on a search engine, e.g. Google. Diverse parameters are entered in the Value Serp API and the query is ran accordingly:

![](0_figures/query_parameters_serp_schema.png)

```{r serp_query_diagram}
...
Here, our query corresponds to the combination of {city name} + {aquatic ecosystem name} for approximately 300 selected agglomerations. A given city can have multiple aquatic ecosystems (e. g. in Lyon, the Rhône and the Saône rivers are both flowing through the city). In the following sections, the aquatic ecosystems are referred to as "rivers" as a simplification.

The following diagram summarizes the flowchart of this document:

![](0_figures/schema_serp_get_data.png){fig-align="center" width="432"}
...
-   pattern: A variable describing roughly the city and its river(s)

    -   confluence: there is a confluence of two important rivers in the city.

    -   multiple: there are multiple water elements in the city.

    -   unique: there is a unique water element.

    -   upstream: there is a confluence of two important rivers upstream from the city.

    -   lake: the main aquatic element is a lake.

    -   coastal: it is a coastal city and there might be a bay or estuary.

    -   specific: it is a specific/particular case which requires more attention or can't be descried with those variables.
...
-   climate: climate zone (GEnS, Metzger et al. 2013 - extracted from BasinATLAS level 06)
...
data_google_domain = read.csv("1_input/glourb-countries.csv")
...
get_serp_data = function(fid, cityname, rivername){
    file = glue::glue("2_get_data/english/raw_data/{fid}.rds")
...
get_serp_tibble = function(fid){
  ...
  file = glue::glue("2_get_data/english/raw_data_text/raw_{fid}.csv")

    if(!file.exists(file)){
# translate to string
    res = readRDS(file = glue::glue("2_get_data/english/raw_data/{fid}.rds"))
    ...
    readr::write_csv(res_tib,
                     glue::glue("2_get_data/english/raw_data_text/raw_{fid}.csv"))
    return(res_tib)
  }
}
```
...
```{r vis_data}
## Lyon
sample(x = read.csv("2_get_data/english/raw_data_text/raw_832.csv")$snippet, size = 10)
## Piura
sample(x = read.csv("2_get_data/english/raw_data_text/raw_647.csv")$snippet, size = 10)
## Bamako
sample(x = read.csv("2_get_data/english/raw_data_text/raw_529.csv")$snippet, size = 10)
```
...
translate_snippets = function(fid){
  ...
  file = glue::glue("2_get_data/english/clean_data/clean_{fid}.csv")
  
  if(!file.exists(file)){
    df = read.csv(glue::glue("2_get_data/english/raw_data_text/raw_{fid}.csv"))
  ...
  readr::write_csv(final_df,
                   glue::glue("2_get_data/english/clean_data/clean_{fid}.csv"))
  }
}
```
...
get_serp_data_hl = function(fid, cityname, rivername, gl, hl, google_domain){
  ...
  file = glue::glue("2_get_data/hl/raw_data/{fid}_{hl}.RDS")
         if(!file.exists(file)){
...
# translate to string
    ...
    file = glue::glue("2_get_data/hl/raw_data_text/raw_{fid}_{hl}.csv")
    
    if(!file.exists(file)){
      res = readRDS(file = glue::glue("2_get_data/hl/raw_data/{fid}_{hl}.RDS"))
      res_text <- httr::content(res, "text")
      ...
      readr::write_csv(res_tib,
                       glue::glue("2_get_data/hl/raw_data_text/raw_{fid}_{hl}.csv"))
...
translate_snippet = function(fid, hl){
  ...
  file1 = glue::glue("2_get_data/hl/clean_data/clean_{fid}_{hl}.csv")
  file2 = glue::glue("2_get_data/hl/clean_data/nodata_{fid}_{hl}.csv")
  
  if(!file.exists(file1) & !file.exists(file2)){
    if(file.exists(glue::glue("2_get_data/hl/raw_data_text/raw_{fid}_{hl}.csv"))){
      df_hl= read.csv(file = glue::glue("2_get_data/hl/raw_data_text/raw_{fid}_{hl}.csv")) 
     
      # check if data has been found beforehand
      if(df_hl[1,1] == "NoData"){
        print(paste0("No data, fid = ",fid, ", hl = ",hl))
        write.csv(df_hl, glue::glue("2_get_data/hl/clean_data/nodata_{fid}_{hl}.csv"))
      }
  ...
  write.csv(final_df, glue::glue("2_get_data/hl/clean_data/clean_{fid}_{hl}.csv"))
      }
    }
    else{
...
for(i in 1:6) {
  ...
  data_subset = filter(data_subset, ((data_subset[, n_city_hl] != "no data") & (data_subset[, n_river_hl] != "no data")))
