...
get_words = function(fid, cityname, rivername){
  ...
  readr::write_csv(df_words,
                   glue::glue("3_occurence/english/lemma_freq/word_{fid}.csv"))
  return()
}
```
...
get_words_hl = function(fid, cityname, rivername, hl){
  ...
  # save the results  
  readr::write_csv(df_words,
                   glue::glue("3_frequency/hl/lemma_freq/word_{fid}_{hl}.csv"))
  return()
}
```
...
show_results = function(fid, cityname, rivername, urban_aggl){
  ...
  # read the data
  freq = read.csv(file = glue::glue("3_occurence/english/lemma_freq/word_{fid}.csv"))
  ...
  # save the plot
  ggsave(filename = paste("plot_",fid,".png"), plot = myplot, path = "3_frequency/english/plots/francais/", scale = 1)
  return()
} 
```
...
show_results_hl = function(cityname, rivername, hl){
  ...
  file = glue::glue("3_occurence/hl/plots/francais/{cityname}_{hl}.png")
  if(!file.exists(file)){
  
  # get the table containing all the languages
  data_language = read.csv("1_input/languages_glourb_code.csv", sep = ";")
  # and select the name of the language we are interested in 
  language = data_language[data_language["code"] == hl, 1]
    
  # read the data
  freq = read.csv(file = glue::glue("3_occurence/hl/lemma_freq/word_{cityname}_{hl}.csv"))
  ...
  png(file = glue::glue("3_occurence/hl/plots/francais/{cityname}_{hl}.png", sep=""), res = 100) # indicate the path to save the plot 
