...
Thanks to the ![](https://dka575ofm4ao0.cloudfront.net/pages-transactional_logos/retina/211386/logo_color_transparent_background.png){width="105" height="17"} API ([access link](https://get.valueserp.com/try-it-free/)), data relative to Google searches have been gathered (see this first [document](https://github.com/lbajemon/glourb_serp)) for approximately 300 combinations of agglomerations and rivers. This document aims to compile the data for all the cities in one single table. This table will be used for further analysis (clustering, topic modelling, specificity score, *etc.*).
...
df = data.frame(data_city_river[,c(1,2, 21)]) %>% 
  mutate(path = paste0("2_get_data/english/tokens/tokens_", fid, ".csv")) %>%  # indicate the path for finding the file for each city
...
write.csv(df, "2_get_data/english/df_tokens.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2)]) %>% 
  mutate(path = paste0("2_get_data/english/clean_data/clean_", fid, ".csv")) %>% 
  mutate(data = map(path, read.csv)) %>% # read the data for each city
  unnest(data) %>%  # create a row for each token for each city, i.e. over 287 000 rows
  mutate(snippet_id = paste0(fid, "_", position)) %>% # create a unique id for each snippet 
...
write.csv(df_compile, "2_get_data/english/df_tokenized_snippets.csv", row.names = FALSE)
...
![](0_figures/Image1.png)

### Compilation of results from the queries ran in the local languages

Let's also do it for the data from the query ran in the local languages.

```{r}
# now put the resulting tokenized sentences in a df containing all of the snippets 
df_compile = data.frame(data_city_river[,c(1,2, 21)]) %>% 
  mutate(path = case_when(
    file.exists(paste0("2_get_data/hl/clean_data/clean_", fid, "_", hl, ".csv")) ~ paste0("2_get_data/english/clean/clean_", fid, ".csv"),
    !file.exists(paste0("2_get_data/hl/clean_data/clean_", fid, "_", hl, ".csv")) ~ "no data"
