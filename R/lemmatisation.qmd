...
lexicon_en = get_lexicon("en") %>% 
  rbind(read.csv("1_input/missing_words_lexicon_en2.csv"))
```
...
# city names 
global_cities = read.csv("1_input/global_cities.csv", sep = ";") 
...
global_admin = read.csv("1_input/global_admin.csv", sep = ",") %>% 
...
lemmatisation_en = function(fid, cityname, rivername){
    ...
    file = glue::glue("2_get_data/english/tokens/tokens_{fid}.csv")
    if(!file.exists(file)){
      
      # read data
      df = read.csv(glue::glue("2_get_data/english/clean_data/clean_{fid}.csv")) 
      ...
      write.csv(df_clean, glue::glue("2_get_data/english/tokens/tokens_{fid}.csv"), row.names = FALSE)
   }
}
```
...
lemmatisation_hl = function(fid, cityname, rivername, hl){
   ...
   file = glue::glue("2_get_data/hl/tokens/tokens_{fid}_{hl}.csv")
    if(!file.exists(file)){
      
      # read data
      if(file.exists(glue::glue("2_get_data/hl/clean_data/clean_{fid}_{hl}.csv"))){
      df = read.csv(glue::glue("2_get_data/hl/clean_data/clean_{fid}_{hl}.csv")) 
      ...
      write.csv(df_clean, glue::glue("2_get_data/hl/tokens/tokens_{fid}_{hl}.csv"), row.names = FALSE)
