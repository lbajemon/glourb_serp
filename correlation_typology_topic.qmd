...
# corpus=read.csv("analysis/clusters/corpus_segments.csv")
# corpus=quanteda::corpus(corpus, docid_field = "x", text_field = "X")
...
plot = res %>%
  ...
  geom_bar(stat = "identity", 
           ...
           width = 0.8, 
           show.legend = FALSE) +
  scale_fill_manual(values = setNames(res$couleur, res$nom)) +
  coord_flip() + 
  labs(x = NULL,
       y = "%",
...
list_clusters = res %>% 
  ...
  select(c("topic", "nom", "couleur")) %>% 
  filter(!is.na(nom))

list_clusters %>% 
  mutate(data = purrr::pmap(list(nom = nom,
                                 cluster = topic,
                                 color = couleur),
                            plot_cluster))
```
...
df_corr = df_corr %>% 
  mutate(topic = case_when(
    topic == "Health" ~ "Santé",
    topic == "Politics, conflicts" ~ "Gouvernement, conflits",
    topic == "Culture, knowledge" ~ "Culture, connaissances",
    topic == "Daily life, news items" ~ "Vie quotidienne, faits divers",
    topic == "Tourism, travel" ~ "Tourisme, voyage",
    topic == "Heritage, attractions" ~ "Patrimoine, attractions",
    topic == "Urban infrastructure and geography" ~ "Infrastructures et géographie urbaines",
    topic == "Online images, web content" ~ "Banques d'images, contenu web",
    topic == "Planning, management, environment" ~ "Planification, gouvernance, environnement",
    topic == "Pollution, sewerage" ~ "Pollution, traitement des eaux usées",
    topic == "Physico-chemical parameters" ~ "Paramètres physico-chimiques",
    topic == "Alert system" ~ "Système d'alerte",
    topic == "Floods" ~ "Inondations, quantité",
    topic == "Hydrography" ~ "Hydrographie")) %>% 
  mutate(typology = case_when(
    typology == "Associative" ~ "Associatif",
    typology ==  "Commercial" ~ "Commercial",
    typology ==  "Cultural" ~ "Culturel",
    typology == "Educational" ~  "Educatif" ,
    typology == "Encyclopaedia & DB"~  "Encyclopédie/BD",
    typology == "Image bank"~ "Banque d’images" ,
    typology ==  "Unknown" ~ "Inconnu",
    typology ==  "Institutionnal" ~ "Institutionnel" ,
    typology == "Book & report" ~ "Livre, rapport",
    typology == "Magazine & blog"  ~ "Magazine, blog",
    typology == "Online press"   ~ "Presse en ligne",
    typology == "Social media" ~ "Réseau social",
    typology == "Scientific" ~ "Scientifique",
    typology == "Utilitarian" ~ "Utilitaire",
    typology == "Wiki" ~ "Wiki"))

order_topics = rev(c("Santé", "Gouvernement, conflits", "Culture, connaissances", "Vie quotidienne, faits divers", "Tourisme, voyage", "Patrimoine, attractions", "Infrastructures et géographie urbaines", "Banques d'images, contenu web", "Planification, gouvernance, environnement", "Pollution, traitement des eaux usées","Paramètres physico-chimiques", "Système d'alerte", "Inondations, quantité", "Hydrographie"))
...
# french
svg(file = "analysis/heatmap_french.svg", width = 5, height = 5)
heatmap(res_matrix, 
        scale = "row", 
        Colv = NA, 
        Rowv = NA, 
