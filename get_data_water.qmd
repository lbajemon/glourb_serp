...
get_serp_data_hl = function(fid, cityname, gl, hl, google_domain){
  ...
  file = glue::glue("collected_data/water_resource/hl/raw_data/{fid}_{hl}.RDS")
    ...
    # let's define our query with a parameters list
           water = polyglotr::google_translate("water", target_language = hl, source_language = "en")
...
# replace some language codes in the table 
data_city_river = data_city_river %>% 
  mutate(hl1 = str_replace_all(hl1, "zh-cn", "zh-CN")) %>% 
  mutate(hl1 = str_replace_all(hl1, "mni", "mni-Mtei")) %>% 
  mutate(hl2 = str_replace_all(hl2, "arz", "no data")) %>% 
  mutate(hl3 = str_replace_all(hl3, "po", "pl"))
