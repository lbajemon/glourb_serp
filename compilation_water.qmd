...
df = data.frame(data_city_river[,c(1,2)]) %>% 
  mutate(path = paste0("collected_data/water_resource/english/tokens/tokens_", fid, ".csv")) %>%  # indicate the path for finding the file for each city
...
df = df[,c(1,2,4:10,25,30,31,42)]
write.csv(df, "collected_data/water_resource/english/df_tokens.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2)]) %>% 
  mutate(path = paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv")) %>% 
  mutate(data = map(path, read.csv)) %>% # read the data for each city
  unnest(data) %>%  # create a row for each token for each city, i.e. over 287 000 rows
  mutate(snippet_id = paste0(fid, "_", position)) %>% # create a unique id for each snippet 
...
df_compile = df_compile[,c(1:2,4:10,25,36:38)]
...
write.csv(df_compile, "collected_data/water_resource/english/df_tokenized_snippets.csv", row.names = FALSE)
...
df = data.frame(data_city_river[,c(1,2,7,8)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl1, ".csv")) %>% # indicate the path for finding the file for each city
  ...
  # if there is no data and the country is the USA, then data can be found in another folder
                          !file.exists(path) & gl == "us" & hl1 != "no data" ~ paste0("collected_data/water_resource/english/tokens/tokens_", fid, ".csv"),
  # else write "no data"
...
no_data = df %>% 
  filter(is.na(path)) %>% 
  select(-c("path")) 
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl1.csv", row.names = FALSE)
...
df = df %>% 
  ...
  mutate(snippet_id = paste0(fid, "_", position)) # create a unique id for each snippet
df = df[,c(1:4,7:12,19,24,25,46)]
write.csv(df, "collected_data/water_resource/hl/df_tokens_hl1.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2,7,8)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/clean_data/clean_", fid, "_", hl1, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl1 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) %>% 
...
# simplify the resulting dataframe and save it
df_compile = df_compile[,c(1:4,7:12,19,40:42)]

colnames(df_compile) = c("fid","urban_aggl","gl","hl","position","title","link","domain","displayed_link","snippet","trans_snippet","snippet_id","tokenized","tokenized_noloc")
write.csv(df_compile, "collected_data/water_resource/hl/df_tokenized_hl1.csv", row.names = FALSE)
```
...
df = data.frame(data_city_river[,c(1,2,7,9)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl2, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl2 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA))
...
no_data = df %>% 
  ...
  select(-c("path"))
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl2.csv", row.names = FALSE)
...
write.csv(df, "collected_data/water_resource/hl/df_tokens_hl2.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2,7,9)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/clean_data/clean_", fid, "_", hl2, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl2 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) %>% 
  filter(!is.na(path)) %>% 
...
df_compile = df_compile[,c(1:4,7:12,28,41:43)]

colnames(df_compile) = c("fid","urban_aggl","gl","hl","position","title","link","domain","displayed_link","snippet","trans_snippet","snippet_id","tokenized","tokenized_noloc")
write.csv(df_compile, "collected_data/water_resource/hl/df_tokenized_hl2.csv", row.names = FALSE)
```
...
df = data.frame(data_city_river[,c(1,2,7,10)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl3, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl3 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA))

no_data = df %>% 
  filter(is.na(path) & hl3 != "no data") %>% 
  select(-c("path"))
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl3.csv", row.names = FALSE)
...
df = df %>% 
  ...
  unnest(data) %>% 
  mutate(snippet_id = paste0(fid, "_", position)) 
df = df[,c(1:4,7:12,20,25,26,46)]
write.csv(df, "collected_data/water_resource/hl/df_tokens_hl3.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2,7,10)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/clean_data/clean_", fid, "_", hl3, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl3 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) %>% 
  filter(!is.na(path)) %>%
...
df_compile = df_compile[,c(1:4,7:12,20,40:42)]

colnames(df_compile) = c("fid","urban_aggl","gl","hl","position","title","link","domain","displayed_link","snippet","trans_snippet","snippet_id","tokenized","tokenized_noloc")
write.csv(df_compile, "collected_data/water_resource/hl/df_tokenized_hl3.csv", row.names = FALSE)
```
...
df = data.frame(data_city_river[,c(1,2,7,11)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl4, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl4 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA))

no_data = df %>% 
  filter(is.na(path) & hl4 != "no data") %>% 
  select(-c("path"))
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl4.csv", row.names = FALSE)
...
df = df[,c(1:4,7:12,26,31,32,37)]
write.csv(df, "collected_data/water_resource/hl/df_tokens_hl4.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2,7,11)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/clean_data/clean_", fid, "_", hl4, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl4 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) %>% 
  filter(!is.na(path)) %>% 
...
df_compile = df_compile[,c(1:4,7:12,26,31:33)]

colnames(df_compile) = c("fid","urban_aggl","gl","hl","position","title","link","domain","displayed_link","snippet","trans_snippet","snippet_id","tokenized","tokenized_noloc")
write.csv(df_compile, "collected_data/water_resource/hl/df_tokenized_hl4.csv", row.names = FALSE)
```
...
df = data.frame(data_city_river[,c(1,2,7,12)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl5, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl5 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA))

no_data = df %>% 
  filter(is.na(path) & hl5 != "no data") %>% 
  select(-c("path"))
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl5.csv", row.names = FALSE)
...
df = df[,c(1:4,7:11,15,26,31:33)]
write.csv(df, "collected_data/water_resource/hl/df_tokens_hl5.csv", row.names = FALSE)
```
...
df_compile = data.frame(data_city_river[,c(1,2,7,12)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/clean_data/clean_", fid, "_", hl5, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl5 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) %>% 
  filter(!is.na(path)) %>% 
...
df_compile = df_compile[,c(1:4,7:11,15,26:29)]

colnames(df_compile) = c("fid","urban_aggl","gl","hl","position","title","link","domain","displayed_link","snippet","trans_snippet","snippet_id","tokenized","tokenized_noloc")
write.csv(df_compile, "collected_data/water_resource/hl/df_tokenized_hl5.csv", row.names = FALSE)
```
...
df = data.frame(data_city_river[,c(1,2,7,13)]) %>% 
  mutate(path = paste0("collected_data/water_resource/hl/tokens/tokens_", fid, "_", hl6, ".csv")) %>% 
  mutate(path = case_when(file.exists(path) ~ path,
                          !file.exists(path) & gl == "us" & hl6 != "no data" ~ paste0("collected_data/water_resource/english/clean_data/clean_", fid, ".csv"),
                          !file.exists(path) & gl != "us" ~ NA)) 

no_data = df %>% 
  filter(is.na(path) & hl6 != "no data") %>% 
  select(-c("path"))
write.csv(no_data, "collected_data/water_resource/hl/no_data_hl6.csv", row.names = FALSE)
...
df1 = read.csv("collected_data/water_resource/hl/df_tokens_hl1.csv") %>% 
  rename_at("hl1", ~"hl")
df2 = read.csv("collected_data/water_resource/hl/df_tokens_hl2.csv") %>% 
  rename_at("hl2", ~"hl")
df3 = read.csv("collected_data/water_resource/hl/df_tokens_hl3.csv") %>% 
  rename_at("hl3", ~"hl")
df4 = read.csv("collected_data/water_resource/hl/df_tokens_hl4.csv") %>% 
  rename_at("hl4", ~"hl")
df5 = read.csv("collected_data/water_resource/hl/df_tokens_hl5.csv") %>% 
  rename_at("hl5", ~"hl")
...
write.csv(df, "collected_data/water_resource/hl/df_tokens_all_hl.csv", row.names = FALSE)
```
...
df = read.csv("collected_data/water_resource/hl/df_tokenized_hl1.csv") %>% 
  rbind(read.csv("collected_data/water_resource/hl/df_tokenized_hl2.csv")) %>% 
  rbind(read.csv("collected_data/water_resource/hl/df_tokenized_hl3.csv")) %>% 
  rbind(read.csv("collected_data/water_resource/hl/df_tokenized_hl4.csv")) %>% 
  rbind(read.csv("collected_data/water_resource/hl/df_tokenized_hl5.csv")) %>% 
  mutate(snippet_id = paste0(fid, "_", hl, "_", position))

write.csv(df, "collected_data/water_resource/hl/df_tokenized_all.csv", row.names = FALSE)
```
