...
# reshape the table
df = dcast(calc, fid_fusion ~ topic, value.var = "perc") %>% 
...
# get cluster labels 
topic_labels = read.csv("input_data/topic_names.csv", sep = "\t")
```

### Map (with topic colors)

```{r def_map}
gradient_map = function(mytopic, mylabel, col1, col2){
...
world_map %>% 
  ...
  ggsave(paste0("analysis/clusters/gradient_maps/topic_colors/map_", mytopic, ".png"), width = 4000, height = 2500, units = "px")
}
```

```{r run_map}
map2(paste0("topic_", 1:14),
     1:14,
     ~gradient_map(mytopic = .x,
                   mylabel = topic_labels$name[.y],
                   col1 = topic_labels$col_low[.y],
                   col2 = topic_labels$col_high[.y]))
```

### Map (with uniform colors)

```{r}
gradient_map2 = function(mytopic, mylabel){
...
world_map %>% 
  ...
  ggsave(paste0("analysis/clusters/gradient_maps/uni_colors/map_", mytopic, ".png"), width = 4000, height = 2500, units = "px")
}

map2(paste0("topic_", 1:14),
     1:14,
     ~gradient_map2(mytopic = .x,
                   mylabel = topic_labels$name[.y]))
```
