...
data_hl = read.csv("collected_data/hl/df_scrap_tokens_all_hl.csv") # this dataframe combines all the collected data 
data_all = read.csv("collected_data/combined_all_tokens.csv")
names(data_hl)[names(data_hl) == 'domain'] = 'full_domain' # rename the "domain" column to avoid confusions 
names(data_all)[names(data_all) == 'domain'] = 'full_domain' # rename the "domain" column to avoid confusions 
...
df_domain_hl = data_hl %>% 
  mutate(domain = suffix_extract(data_hl$full_domain)) %>%
  group_by(domain$domain) %>% # group by domain 
  count() %>% # count each domain
  filter(n > 100) %>% 
  left_join(domain_type, join_by("domain$domain" == "domain"))
# rename column
names(df_domain_hl)[1] = "domain"

# rearrange by decreasing order
df_domain_hl$domain = with(df_domain_hl, reorder(domain, n))
...
ggsave("analysis/typology/ranking_typo_hl.png")
```

#### For local languages + English

```{r}
### now let's rank the websites for all of our data (english + hl)
df_domain_all = data_all %>%
  mutate(domain = suffix_extract(data_all$full_domain)) %>%
  group_by(domain$domain) %>% # group by domain 
  count() %>% # count each domain
  filter(n > 130) %>% 
  left_join(domain_type, join_by("domain$domain" == "domain"))
# rename column
names(df_domain_all)[1] = "domain"

# rearrange by decreasing order
df_domain_all$domain = with(df_domain_all, reorder(domain, n))
...
plot = df_domain_all %>% 
  ...
  geom_col(position = "stack") +
  scale_fill_manual("Type de site", values = palette) +
  labs(title = "Classement des domaines pour les requêtes rivière+ville",
       subtitle = "(langues locales et anglais)",
       x = "nom du domaine",
       y = "nombre de pages") +
  coord_flip() + # flip x and y coordinates 
  theme_bw() +
  theme(legend.position = "bottom",
        plot.caption.position = "plot") 
plot # display the plot
ggsave("analysis/typology/ranking_typo_all.png")
ggsave("analysis/typology/ranking_typo_all_paysage.png", width = 22, height = 16, unit = "cm")
```
...
bind = function(df){
  data = read.csv(glue::glue("analysis/etude_10_villes/{df}.csv"))
  data = data %>% 
...
df_en_types = results_en %>% 
  ...
  mutate(type = str_replace_all(type, "commercial, entreprises", "commercial, entreprise"))
df_en_types$type = gsub("académique(?!, scientifique)", "académique, scientifique", df_en_types$type, perl = TRUE)
df_en_types$type = trimws(df_en_types$type)

write.csv(df_en_types, "analysis/typology/df_en_types.csv")
...
df_hl_types = results_hl %>% 
  ...
  mutate(type = str_replace_all(type, "entreprise, commerce", "commercial, entreprise")) %>% 
  mutate(type = str_replace_all(type, "commercial, entreprises", "commercial, entreprise"))
df_hl_types$type = gsub("académique(?!, scientifique)", "académique, scientifique", df_hl_types$type, perl = TRUE)
df_hl_types$type = trimws(df_hl_types$type)
write.csv(df_hl_types, "analysis/typology/df_hl_types.csv")

# make a df summarising the identified types 
types_hl = df_hl_types %>% 
  filter(!is.na(type)) %>% 
  distinct(domain, .keep_all = TRUE) %>% 
  select("domain", "type") 

types_en = df_en_types %>% 
  filter(!is.na(type)) %>% 
  distinct(domain, .keep_all = TRUE) %>% 
  select("domain", "type")
...
types = types_hl %>% 
  rbind(types_en) %>% 
  distinct(domain, .keep_all = TRUE)
write.csv(types, "analysis/typology/tous_sites_types.csv")
```
...
plot_en_1 = somme_en %>% 
  ...
  labs(x = NULL,
       ...
       caption = "L. Bajemon, septembre 2024") +
  theme_bw(base_family = "CenturySch", 
           base_size = 12) +
  theme(plot.caption.position = "plot")
ggsave("analysis/typology/repartition_id_en.png")
...
ggsave("analysis/typology/types_EN.png", width = 20, height = 10, units = "cm")
```

```{r plot_hl}
# count nb of values for each type
types_hl_plot = df_hl_types %>% 
  mutate(type = str_replace_all(type, "journal", "média")) %>% 
  mutate(type = str_replace_all(type, "magazine, blog ", "magazine, blog")) %>% 
  mutate(type = str_replace_all(type, "ONG, association", "associatif, ONG")) %>% 
  mutate(type = str_replace_all(type, "forum", "réseau social")) %>% 
  dplyr::count(type) 
...
plot_hl_1 = somme_hl %>% 
  ...
  theme_bw(base_family = "CenturySch", 
           base_size = 12) +
  theme(plot.caption.position = "plot")
ggsave("analysis/typology/repartition_id_hl.png")
...
plot_hl_2 = identified_hl %>% 
  ...
  theme_bw(base_family = "CenturySch", 
           base_size = 12) +
  theme(plot.caption.position = "plot")  
ggsave("analysis/typology/types_HL.png", width = 20, height = 10, units = "cm")
```

Now let's combine the two plots showing the type for English and local queries:

```{r combine_plots}
library(patchwork)

plot_1 = identified_en %>% 
  mutate(type = fct_reorder(type, perc)) %>% 
  ggplot(aes(x = type, y = perc)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8, fill = "#b3de69") + 
  coord_flip() + # flip x and y coordinates 
  labs(x = NULL,
       y = "%",
       title = "Requêtes en anglais") +
  theme_bw(base_family = "CenturySch", 
           base_size = 12) +
  theme(plot.caption.position = "plot") 
plot_2 = identified_hl %>% 
  mutate(type = fct_reorder(type, perc)) %>% 
  ggplot(aes(x = type, y = perc)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8, fill = "#b3de69") + 
  coord_flip() + # flip x and y coordinates 
  labs(x = NULL,
       y = "%",
       title = "Requêtes dans les langues locales") +
  theme_bw(base_family = "CenturySch", 
           base_size = 12) +
  theme(plot.caption.position = "plot")  

combined_plot = plot_1 + plot_2
ggsave("analysis/typology/figure_typo_DSSE.png", width = 28, height = 15, units = "cm")
```
