...
data_city_river_303 = read.csv("input_data/data_city_river_303.csv") %>% 
  select("fid", "latitude", "longitude", "urban_aggl") %>% 
  mutate(latitude = as.numeric(str_replace_all(latitude, ",", ".")),
...
# get number of pages for each agglomeration
nb_tot = corpus %>% 
  mutate(requete = paste0(hl, "-", query)) %>% 
  group_by(fid) %>% 
  summarise(n_langues = n_distinct(requete)) %>% 
  # get number of requests for each agglomeration
  left_join(fid_fusion, by = "fid") %>% 
  group_by(fid_fusion) %>% 
  summarise(n_total = sum(n_langues))

corpus_fusion = corpus %>%
  left_join(fid_fusion, by = "fid") %>% 
  group_by(fid_fusion) %>% 
  dplyr::summarise(n = n()) %>% 
  dplyr::rename(n_pages = n) %>% 
  left_join(data_city_river_303, by = join_by("fid_fusion" == "fid")) %>% 
  left_join(nb_tot, by = "fid_fusion") %>% 
  mutate(moy = n_pages/n_total)

write.csv(corpus_fusion, "analysis/description_corpus/nb_pages_par_agglo.csv")

# get number of pages for each city-river combination
nb_tot = corpus %>% 
  mutate(requete = paste0(hl, "-", query)) %>% 
  group_by(fid) %>% 
  summarise(n_langues = n_distinct(requete))
...
nb_per_combi = corpus %>%
  ...
  left_join(nb_tot, by = "fid") %>% 
  mutate(moy = n_pages/n_langues)

write.csv(nb_per_combi, "analysis/description_corpus/nb_pages_combi.csv")

### MAP 
data_sf = st_as_sf(corpus_fusion, coords = c("longitude", "latitude"), crs = 4326)

## Table by subcontinent
...
data_city_river = read.csv("input_data/data_city_river_303.csv") %>% 
  select(c("fid", "sub_continent"))

subcontinent = corpus_fusion %>% 
  left_join(data_city_river, by = join_by("fid_fusion" == "fid")) %>% 
  group_by(sub_continent) %>% 
  mutate(moy_cont = mean(moy),
         max_cont = max(moy),
         min_cont = min(moy),
         med_cont = median(moy)) %>% 
  ungroup() %>% 
  left_join(noms_conti, by = "sub_continent") %>% 
  select(c("nom", "moy_cont", "max_cont", "min_cont", "med_cont")) %>% 
  unique() %>% 
  arrange(nom) 

colnames(subcontinent) = c("Sous-continent", "Moyenne", "Maximum", "Minimum", "Médiane")
```
...
base_map %>% 
  ...
  theme_void() +
  theme(legend.position = "bottom") +
  labs(color = "Nombre moyen de pages")

ggsave("analysis/description_corpus/nb_moyen_pages.svg", width = 4000, height = 2500, units = "px")

# manual scale colour
data_sf = data_sf %>% 
  mutate(classe = case_when(
    moy <= 25 ~ "≤ 25 pages",
    moy > 25 & moy <= 50 ~ "26-50 pages",
    moy > 50 & moy <= 75 ~ "51-75 pages",
    moy > 75 & moy <= 98 ~ "76-98 pages",
    moy > 98 ~ "99 pages"))
...
base_map %>% 
  ...
  geom_sf(data = data_sf, 
          ...
          alpha = 1,
          size = 3) +
  scale_color_manual(values = c("#FDE725FF", "#73D055FF", "#1F968BFF", "#33638DFF", "#440154FF"),
                     breaks = c("≤ 25 pages", "26-50 pages", "51-75 pages", "76-98 pages", "99 pages")) +   # equal earth projection
  coord_sf(crs = "+proj=eqearth") +
  # theme
  theme_void() +
  theme(legend.position = "bottom",
  ...
  labs(color = "Nombre moyen de pages\nobtenu par agglomération",
       tag = "Fond de carte : R Natural Earth. Réalisation : L. Bajemon, décembre 2025.")

ggsave("analysis/description_corpus/nb_moyen_pages_cat.svg", width = 4000, height = 2500, units = "px")
```

```{r}
### violin chart by subcontinent
# data
data_cont = corpus_fusion %>% 
  left_join(data_city_river, by = join_by("fid_fusion" == "fid")) %>% 
  left_join(noms_conti, by = "sub_continent") %>% 
  unique() %>% 
  arrange(nom) 

data_cont$nom = factor(data_cont$nom, levels = c("Afrique", "Europe", "Amérique du Nord", "Amérique centrale", "Amérique du Sud", "Asie de l'Ouest", "Asie du Sud-Est", "Chine", "Inde"))

data_cont %>% 
  ggplot(aes(x = nom, 
             y = moy)) +
  geom_boxplot(fill = "#3CBB75FF",
               color = "black") +
  theme_bw() +
  labs(y = "",
       x = "")

ggsave("analysis/description_corpus/nb_moyen_pages_boxplot.svg", width = 1700, height = 800, units = "px")
```
...
world_map %>% 
  ...
  scale_color_manual(breaks = c("Aucune page", "1 à 3 pages", "4 à 5 pages", "6 à 10 pages", "Plus de 10 pages"),
                     values = c("grey", "#fdd49e", "#fc8d59", "#d7301f", "#990000")) + 
  # equal earth projection
  coord_sf(crs = "+proj=eqearth") +
  # theme
  theme_void() +
