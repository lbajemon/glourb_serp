...
mylabels = NULL 
mypalette = NULL
clusters_18_noloc = run_dhc(df_noloc, dfmatrix_noloc, 18, 500, tokens_noloc, mylabels, big_palette, path = "analysis/clustering/clustering_hl1_k18_noloc.png", measure = "chi2", filename = "18_hl1_noloc")
...
The optimal numbers of clusters are 15 (including locations, more details), 17 (excluding locations, more details) and ... (less details).
...
plot_loc = rainette_plot(
  ...
  show_negative = FALSE,
  text_size = 12,
  cluster_label = c("Outlier \n(vidéos)", "Tourisme", "Internet", "Outlier", "Pertes et \n victimes", "Outlier \n(réseaux sociaux)", "Gestion, \naménagement, \npollution", "Inondation", "Outlier", "Hydrographie", "Distance", "Outlier", "Patrimoine, \nhistoire", "Localisation", "Inde")
  )
...
##### c.2. 17 clusters: without locations

```{r 17_clusters_all_hl}
dhc_noloc = rainette(dfmatrix_noloc, 17, min_split_members = 500, min_segment_size = 0)
...
plot_noloc = rainette_plot(
  ...
  show_negative = FALSE,
  text_size = 12,
  cluster_label = c("Internet et \ntourisme", "Outlier", "Hydrographie", "Distance", "Localisation \nrégion/pays", "Localisation \nville", "Traffic routier", "Outlier", "Gouvernement, \npolitique", "Energie, \nindustrie", "Aménagement, \ngestion et \npollution", "Culture, \nloisirs", "Infrastructures, \nréseaux", "Milieu \nuniversitaire, \nqualité de l'eau", "Pertes et \nvictimes", "Inondation", "Outlier"),
  colors = big_palette
  )
ggsave("analysis/clustering/clustering_all_hl_k17_noloc.png", plot_noloc, width = 500, height = 250, units = "mm", limitsize = FALSE)

# save the results (i.e. most frequent terms for each cluster)
groups_noloc = cutree_rainette(dhc_noloc, k = 15)
groups_stats_noloc = rainette_stats(groups_noloc, dfmatrix_noloc)
list.save(groups_stats_noloc, glue::glue("analysis/clustering/resultats_statistiques_17_all_hl_noloc.rds"))
  
  # for each element of the corpus (i.e. each snippet)
  # write its class according to the clustering
  corpus_noloc$cluster = cutree_rainette(dhc_noloc, 17)
  # create a df indicating the cluster for each doc 
  ...
  df_clusters_noloc = df_noloc %>% 
    left_join(docvar, by = "seg_id", keep = FALSE)
# and save the result
write.csv(df_clusters_noloc, glue::glue("analysis/clustering/classified_snippets_17_all_hl_noloc.csv"), row.names = FALSE)

# summarise results
summary_17_noloc = summary_clusters(df_clusters_noloc)
data_city_river = read.csv("input_data/data_city_river.csv") %>% 
  left_join(summary_17_noloc, by = "fid")
# and save them
write.csv(data_city_river, "analysis/clustering/summary_17_all_hl_noloc.csv", row.names = FALSE)
```

##### c.3. 9 clusters: with locations

```{r 9_clusters_hl}
dhc_loc = rainette(dfmatrix_loc, 9, min_split_members = 500, min_segment_size = 0)
...
plot_loc = rainette_plot(
  ...
  show_negative = FALSE,
  text_size = 12,
  cluster_label = c("Outlier \n(vidéos)", "Tourisme, \nInternet", "Victimes", "Outlier \n(réseaux sociaux)", "Gestion, \naménagement, \npollution", "Inondation", "Outlier", "Hydrographie", "Localisation"),
  colors = mypalette
  )
ggsave("analysis/clustering/clustering_all_hl_k9_loc.png", plot_loc, width = 500, height = 250, units = "mm", limitsize = FALSE)

# save the results (i.e. most frequent terms for each cluster)
groups_loc = cutree_rainette(dhc_loc, k = 9)
groups_stats_loc = rainette_stats(groups_loc, dfmatrix_loc)
list.save(groups_stats_loc, glue::glue("analysis/clustering/resultats_statistiques_9_all_hl_loc.rds"))
  
  # for each element of the corpus (i.e. each snippet)
  # write its class according to the clustering
  corpus_loc$cluster = cutree_rainette(dhc_loc, 9)
  # create a df indicating the cluster for each doc 
  docvar = docvars(corpus_loc) %>% 
    mutate(seg_id = paste0(fid, "_", hl, "_", position)) %>% 
    # seg_id: unique identifier of each segment 
  ...
  df_clusters_loc = df_loc %>% 
    left_join(docvar, by = "seg_id", keep = FALSE)
# and save the result
write.csv(df_clusters_loc, glue::glue("analysis/clustering/classified_snippets_9_all_hl_loc.csv"), row.names = FALSE)

# summarise results
summary_9_loc = summary_clusters(df_clusters_loc)
data_city_river = read.csv("input_data/data_city_river.csv") %>% 
  left_join(summary_9_loc, by = "fid")
# and save them
write.csv(data_city_river, "analysis/clustering/summary_9_all_hl_loc.csv", row.names = FALSE)
```

##### c.4. 10 clusters: without locations
...
plot_noloc = rainette_plot(
  ...
  show_negative = FALSE,
  text_size = 12,
  cluster_label = c("Internet, \ntourisme", "Outlier", "Hydrographie", "Localisation \npays/région", "Localisation \nville", "Traffic routier", "Gestion, \naménagement, \npollution", "Milieu universitaire, \npollution", "Inondation", "Outlier \n(vidéos)"),
  colors = get_brewer_pal("Set2", 10)
  )
ggsave("analysis/clustering/clustering_all_hl_k9_loc.png", plot_loc, width = 500, height = 250, units = "mm", limitsize = FALSE)
