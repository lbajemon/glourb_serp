...
run_dhc = function(df, dfmatrix, n_class, min_members, corpus, mylabels, mycolors){
...
# plot
  ...
  ggsave(glue::glue("analysis/clustering/clustering_k{k}.png"), plot, width = 500, height = 250, units = "mm", limitsize = FALSE)
  ...
  list.save(groups_stats, glue::glue("analysis/clustering/resultats_statistiques_k{k}.rds"))
...
# and save the result
  write.csv(df_clusters, glue::glue("analysis/clustering/classified_snippets_k{k}.csv"), row.names = FALSE)
...
# apply the DHC
# through trial and error, we find that the best k number in this case is 18 and 6 if we want less details
# let's define the cluster names and their color. We advise to run the run_dhc function a first time with mylabels = NULL and mypalette = NULL and then to define labels according to the results
mylabels = NULL 
mypalette = NULL
clusters_18 = run_dhc(df, dfmatrix, 18, 500, corpus, mylabels, mypalette)

mylabels = c("Hydrographie", "Localisation", "Réseau et infrastructures", "Connaissances et gestion", "Tourisme et loisirs", "Internet")
mypalette = get_brewer_pal("Set2", 6)
clusters_6 = run_dhc(df, dfmatrix, 6, 500, corpus, mylabels, mypalette)
...
dataset = read.csv("analysis/clustering/selection_50_cities/clustering_300.csv") # %>%
  left_join(summary_6, by = "fid") %>% 
  left_join(nb_snippets, by = "fid") %>% 
  write.csv("analysis/clustering/selection_50_cities/results_clustering6.csv", row.names = FALSE)
...
# and save them
write.csv(data_city_river, "analysis/clustering/18_clusters_371_cities.csv")
```
