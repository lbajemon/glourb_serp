...
##### c.1. 15 and 18 clusters

```{r 15_18_clusters_hl}
# with locations
mylabels = NULL 
mypalette = NULL
clusters_15_loc = run_dhc(df_loc, dfmatrix_loc, 15, 500, tokens_loc, mylabels, mypalette, path = "analysis/clustering/clustering_all_hl_k15_loc.png", measure = "chi2", filename = "15_all_hl_loc")

# summarise results
summary_15_loc = summary_clusters(clusters_15_loc)
data_city_river = read.csv("input_data/data_city_river.csv") %>% 
  left_join(summary_15_loc, by = "fid")
# and save them
write.csv(data_city_river, "analysis/clustering/summary_15_all_hl_loc.csv")

# without locations
mylabels = NULL 
mypalette = NULL
clusters_17_noloc = run_dhc(df_noloc, dfmatrix_noloc, 17, 500, tokens_noloc, mylabels, mypalette, path = "analysis/clustering/clustering_all_hl_k17_noloc.png", measure = "chi2", filename = "17_all_hl_noloc")

# summarise results
summary_17_noloc = summary_clusters(clusters_17_noloc)
data_city_river = read.csv("input_data/data_city_river.csv") %>% 
  left_join(summary_17_noloc, by = "fid")
# and save them
write.csv(data_city_river, "analysis/clustering/summary_17_all_hl_noloc.csv")
```
...
mylabels = c("Académique", "Politique et aménagement", "Pêche et déchets", "Inondation", "Transports et déplacements", "Hydrographie", "Localisation", "Navigation", "Tourisme et internet", "Outlier")
mypalette = get_brewer_pal("Set2", 10)
clusters_10_loc = run_dhc(df_loc, dfmatrix_loc, 10, 500, tokens_loc, mylabels, mypalette, path = "analysis/clustering/clustering_all_hl_k10_loc.png", measure = "chi2", filename = "10_all_hl_loc")
...
dataset = read.csv("input_data/clustering_300.csv") %>%
  left_join(summary_10_loc, by = "fid") %>% 
  left_join(nb_snippets, by = "fid") %>% 
  write.csv("analysis/clustering/summary_10_all_hl_loc.csv", row.names = FALSE)
  
# no loc
mylabels = c("Internet", "Outlier", "Hydrographie", "Localisation", "Réseaux et ville", "Politique et aménagement", "Inondation", "Outlier")
mypalette = get_brewer_pal("Set2", 8)
clusters_8_noloc = run_dhc(df_noloc, dfmatrix_noloc, 8, 500, tokens_noloc, mylabels, mypalette, path = "analysis/clustering/clustering_all_hl_k8_noloc.png", measure = "chi2", filename = "8_all_hl_noloc")

# summarise results
summary_8_noloc = summary_clusters(clusters_8_noloc) %>% 
  separate(fid, c("fid", "supp_fid"), sep = "_") %>% 
  group_by(fid) %>% 
  dplyr::mutate(n = n()) %>% 
  ungroup() %>% 
